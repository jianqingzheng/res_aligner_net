...
from ..pytools import patchlib as pl
from ..pytools import timer as timer
...
def _eq_diffs(y,ndims=3):
    ...
    for i in range(ndims):
        ...
        yt = tf.transpose(y, [d, *range(d), *range(d + 1, ndims + 1)])
        # dfi = y[1:, ...] - y[:-1, ...]
        ...
        df[i] = tf.transpose(tf.pad(yt[1:, ...] - yt[:-1, ...], pad), [*range(1, d + 1), 0, *range(d + 1, ndims + 1)])
    return df
# ================================<diff

def interpn(vol, loc, interp_method='linear',weight_vol=None): ###===============<weight_vol
    """
    ...
    ####===================================================> ASTN
        ...
        # weights_loc = [diff_loc1, diff_loc0]  # note reverse ordering since weights are inverse of diff.
        weights_loc = [[None for _ in range(nb_dims)] for _ in range(2)]
        diff_vol = [tf.abs(v[...,0]) for v in _eq_diffs(vol)]
        for dim_id in range(nb_dims):
            # for x1,x0 in zip(diff_loc1,diff_loc0):
            diff_exp_loc0 = tf.exp(scale_num * diff_vol[dim_id] * diff_loc0[dim_id])-1
            diff_exp_loc1 = tf.exp(scale_num * diff_vol[dim_id] * diff_loc1[dim_id])-1

            exp_sum=diff_exp_loc0+diff_exp_loc1+1
            weights_loc[0][dim_id] = (diff_loc1[dim_id]+diff_exp_loc1)/exp_sum
            weights_loc[1][dim_id] = (diff_loc0[dim_id]+diff_exp_loc0)/exp_sum
        ...
        # weights_loc = [diff_loc1, diff_loc0]  # note reverse ordering since weights are inverse of diff.
        weights_loc = [[None for _ in range(nb_dims)] for _ in range(2)]
        diff_vol = tf.clip_by_value(tf.abs(weight_vol),clip_value_max=20,clip_value_min=0)
        for dim_id in range(nb_dims):
            # for x1,x0 in zip(diff_loc1,diff_loc0):
            diff_exp_loc0 = tf.exp(diff_vol[...,dim_id] * diff_loc0[dim_id])-1
            diff_exp_loc1 = tf.exp(diff_vol[...,dim_id] * diff_loc1[dim_id])-1
...
def transform(vol, loc_shift, interp_method='linear', indexing='ij',weight_vol=None): ###=========
    """
    ...
    # test single
    return interpn(vol, loc, interp_method=interp_method,weight_vol=weight_vol)
